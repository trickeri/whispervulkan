...
# "ram" forces CPU inference (--no-gpu) so the weights live in system RAM.
DEVICE_FILE="${XDG_CACHE_HOME:-$HOME/.cache}/modelmanager/whispermodel.device"
WV_DEVICE="$( [ -f "$DEVICE_FILE" ] && cat "$DEVICE_FILE" || echo gpu )"
...
ARGS=(--model "$WV_MODEL" --host "$WV_HOST" --port "$WV_PORT"
      --threads "$WV_THREADS" --language "$WV_LANG"
      --inference-path /inference --convert)
[ "$WV_DEVICE" = ram ] && ARGS+=(--no-gpu)

echo "whispermodel: $WV_MODEL on $WV_HOST:$WV_PORT (${WV_THREADS}t, lang=$WV_LANG, device=$WV_DEVICE)" >&2
# whisper-server is rpath-linked to libwhisper/libggml*.so in its build dir — exec in place.
exec "$WHISPER_SERVER_BIN" "${ARGS[@]}"
